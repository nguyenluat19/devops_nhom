...
const Reviews = () => {
    ...
    useEffect(() => {
        fetchReviews(); // Gọi hàm để lấy bình luận khi component được mount
    }, []);
            ...
            <div id="review-list">
                {reviews.map((review) => (
                    <div key={review._id} className="review">
                        <p><strong>{review.user.name}</strong> (Email: {review.user.email}): {review.comment}</p>
                        <p>Rating: {review.rating}</p>

                        {/* Form trả lời bình luận */}
                        <form onSubmit={(e) => { e.preventDefault(); replyToReview(review._id); }}>
                            <textarea
